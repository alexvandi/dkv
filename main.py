...
def index():
    global last_excel_path, last_excel_name
    if request.method == "POST":
        ...
        if uploaded_file and uploaded_file.filename.endswith(".pdf"):
            ...
            last_excel_path, last_excel_name = extract_and_generate_excel(
                pdf_path, pdf_name)
...
def download_excel():
    if os.path.exists(last_excel_path):
        return send_file(last_excel_path,
                         as_attachment=True,
                         download_name=last_excel_name)
